...
from core.config.settings import get_llm_config, get_vlm_config
...
class BaseModelPool(ABC, Generic[T]):
    ...
    async def _fetch_model_data(self, model_name: str) -> dict[str, Any]:
        ...
        elif model_name == vlm.model_name:
            apikey = os.getenv("VLM_API_KEY")
            return {
                "model_name": vlm.model_name,
                "category": ModelCategory.VLM.value,
                "provider": vlm.provider,
                "api_endpoint": vlm.api_endpoint,
                "api_key": apikey,
    ...
    async def _fetch_available_models(self) -> list[dict[str, Any]]:
        ...
        if model_category == ModelCategory.LLM.value:
            config = get_llm_config()
        elif model_category == ModelCategory.VLM.value:
            config = get_vlm_config()
        else:
            logger.warning(f"未知的模型类别: {model_category}")
            return []
        return [{

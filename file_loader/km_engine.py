...
from .km_meta import KMFileMetaService
...
class KmEngine:
    ...
    - 优雅的关闭机制
    """
    def __init__(self, parallel_workers: int = 2, check_interval: int = 600):
        """初始化文件解析引擎
        
        Args:
            parallel_workers: 并行工作线程数，默认2个
            check_interval: 数据库轮询间隔（秒），默认600秒
        """
        ...
        self.km_meta = KMFileMetaService()
    ...
    async def _producer_loop(self):
        ...
        while True:
            try:
                ...
                pending_files = await self.km_meta.retrieve_asset_metadata(offset=0, limit=100, processed_flag="N")
                
                for km_meta in pending_files:
                    asset_id = km_meta.asset_id
                    
                    if asset_id not in self.processing_ids:
                        self.processing_ids.add(asset_id)
                        # 入队（队列满时异步阻塞 - 实现背压控制）
                        timestamp = datetime.now()
                        await self.queue.put((timestamp, km_meta))
                        logger.debug(f"Asset {asset_id} 成功入队")
    ...
    async def _worker_loop(self, worker_id: int):
        ...
        while True:
            ...
            try:
                ...
                timestamp, km_meta = queue_item
                asset_id = km_meta.asset_id
                
                logger.info(f"工作线程-{worker_id} 开始处理Asset：{asset_id}")
                
                # 执行核心解析逻辑
                await self.processor.process_asset(km_meta)
                
                logger.info(f"工作线程-{worker_id} 完成Asset处理：{asset_id}， 耗时：{datetime.now() - timestamp} 秒")
